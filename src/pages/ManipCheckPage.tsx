...
import { manipCheckCore, manipMeetingTypeQuestion } from '../data/scales/manipulationCheck'
...
export default function ManipCheckPage() {
  ...
  const saved = useExperimentStore((state) => state.dvResponses.manipCheck as ManipCheckState | undefined)
  ...
  const allAnswered = useMemo(() => {
    return manipCheckCore.every((item) => values[item.id] !== null) && Boolean(values.mc_meeting_type)
  }, [values])
    ...
    <section className="mx-auto max-w-5xl space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="space-y-4">
        ...
        <article className="rounded-xl border border-slate-200 bg-white p-4">
          <p className="mb-3 text-sm text-slate-800">{manipMeetingTypeQuestion.text}</p>
      ...
      {showError && <p className="text-sm text-rose-600">请完成所有题目后再进入下一页。</p>}
