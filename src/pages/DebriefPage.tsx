...
export default function DebriefPage() {
  ...
  const receiptConfirmed = useExperimentStore(
    (state) => state.receiptConfirmed,
  );
  const receiptViewDuration = useExperimentStore(
    (state) => state.receiptViewDuration,
  );
  ...
  const moralDisengagementScale = useExperimentStore(
    (state) => state.moralDisengagementScale,
  );
  const moralIdentityScale = useExperimentStore(
    (state) => state.moralIdentityScale,
  );
  ...
  const finalPayload = useMemo(
    () => ({
      ...
      dv: {
        ...
        moralIdentityScale,
        fillerScale: dvResponses.fillerScale ?? null,
        moralCredit: dvResponses.moralCredit ?? null,
  ...
  const submitFinalData = useCallback(async () => {
    ...
    const endpoint =
      (import.meta.env.VITE_FINAL_SUBMIT_ENDPOINT as string | undefined) ??
      "/api/final-submit";
    ...
    try {
      ...
      localStorage.setItem(
        "experiment_final_submission_fallback",
  ...
  return (
    <section className="mx-auto max-w-3xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4 text-sm">
        ...
        {status === "success" && (
          <p className="text-center text-emerald-700">✅ 数据已成功提交</p>
        )}
        {status === "error" && (
